...
public class WsMonitorHandler extends WsBaseHandler<WsMonitorEvent> {
    ...
    public void handleEvent(WsMonitorEvent event) {
        ...
        if (monitor.getAgentType() != 2) {
            sendMessgae(event, new WsResponseEntity<String>(ErrorCode.ACCOUNT_AUTH_ERROR, event.getCmd(), event.getAgentKey()));
            return;
        }
        ...
        sendMessgae(event, new WsResponseEntity<>(AgentState.MONITOR.name(), event.getAgentKey()));
    }
        ...
        scheduledExecutorService.scheduleAtFixedRate(() -> {
            ...
            while (iterator.hasNext()) {
                ...
                for (Long groupId : groupIds) {
                    ...
                    sendMessgae(monitorEvent, monitorVos);
                }
            }
